...
# Sanity: new bundle must include Insights + Thoughts foyer routes
JS="$(ls assets/index-*.js | head -1)"
if ! grep -q '/insights' "$JS"; then
  echo "error: built bundle $JS missing /insights — aborting publish." >&2
  exit 1
fi
if ! grep -q '/thoughts/passing' "$JS" || ! grep -q '/thoughts/longer' "$JS"; then
  echo "error: built bundle $JS missing Thoughts foyer rooms — aborting publish." >&2
  exit 1
fi
if grep -q 'to:"/writing",label:"Writing"' "$JS"; then
  echo "error: built bundle $JS still has Writing as a nav tab — aborting publish." >&2
  exit 1
fi
echo "OK: $JS includes /insights and Thoughts foyer (no Writing nav tab)"
